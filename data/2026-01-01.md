...
### [1] [Graph quandles: Generalized Cayley graphs of racks and right quasigroups](https://arxiv.org/abs/2506.04437)
*Luc Ta*
...
Motivation: 解决关于rack的Cayley图和右拟群的图论实现的两个开放问题，扩展标记Cayley有向图的分类理论到更广泛的代数结构。

Method: 1. 刻画实现rack的图的标记；2. 通过图的置换表示构造rack理论图不变量；3. 证明所有右拟群都可以通过无边图和完全图实现；4. 使用Schreier图刻画实现右拟群的Cayley图；5. 给出右消去幺半群、右可除幺半群、右拟群、rack、quandle、对合rack和kei的标记Cayley有向图的图论特征。

Result: 1. 完全刻画了实现rack的图的标记；2. 构造了rack理论图不变量并给出了路径图和循环图的计算方法；3. 证明了所有右拟群都可以通过无边图和完全图实现；4. 刻画了实现右拟群的Cayley图；5. 给出了多种代数结构的标记Cayley有向图的完整图论特征。

Conclusion: 该论文成功解决了两个开放问题，将Cayley图理论扩展到右拟群和相关代数结构，建立了代数结构与图论之间的新联系，为研究这些代数结构提供了新的图论工具和方法。
...
### [2] [Twisted Cherednik systems and non-symmetric Macdonald polynomials](https://arxiv.org/abs/2512.24811)
*A. Mironov,A. Morozov,A. Popolitov*

Main category: hep-th

TL;DR: 该论文研究多体系统哈密顿量的本征函数，这些哈密顿量源于DIM代数的交换子代数，本征函数可通过非对称Macdonald多项式构造，其展开具有普适系数，为可积系统理论提供了新维度。


<details>
  <summary>Details</summary>
Motivation: 研究广义(a-扭曲)Cherednik算子相关多体系统哈密顿量的本征函数，探索非对称多项式在可积系统理论中的作用，扩展传统对称多项式理论框架。

Method: 基于DIM代数的交换子代数构造哈密顿量，利用扭曲Baker-Akhiezer函数作为基态本征函数，通过非对称Macdonald多项式构造一般本征函数，其展开具有普适系数。

Result: 建立了从非对称Macdonald多项式构造本征函数的通用公式，揭示了本征函数展开中系数与扭曲参数a的分离特性，为可积系统理论提供了新的数学结构。

Conclusion: 该研究为可积系统理论引入了非对称多项式的新视角，扩展了传统对称多项式、非扭曲Baker-Akhiezer函数和Noumi-Shiraishi级数构成的"三元组"理论框架。
...
### [3] [Generalized Level-Rank Duality, Holomorphic Conformal Field Theory, and Non-Invertible Anyon Condensation](https://arxiv.org/abs/2512.24419)
*Clay Cordova,Diego García-Sepúlveda,Jeffrey A. Harvey*

Main category: hep-th

TL;DR: 该论文研究了全纯共形场论与3D拓扑量子场论对偶性之间的关联，利用Schellekens分类的c=24全纯理论发现了新的零星对偶性，其中一些涉及非阿贝尔任意子凝聚。


<details>
  <summary>Details</summary>
Motivation: 研究全纯共形场论与3D拓扑量子场论对偶性之间的相互作用，特别是推广水平-秩对偶范式，探索通过全纯共形场论中的Kac-Moody子代数产生的拓扑界面如何导致拓扑场论之间的对偶性。

Method: 利用具有Kac-Moody子代数的全纯共形场论构造Chern-Simons规范理论之间的拓扑界面，通过凝聚合适的任意子集合，从这些界面推导出拓扑场论之间的对偶性。具体使用Schellekens分类的c=24全纯理论作为例证。

Result: 发现了一系列新的零星对偶性，其中一些必须涉及非阿贝尔统计任意子的凝聚（即规范非可逆的一形式全局对称性）。部分例子从c=24推广到无限系列，包括Spin(n²)₂与扭曲二面群规范理论的对偶性。当-1是模k的二次剩余时，推导出在中心荷c=2(k-1)处存在一系列具有融合范畴对称性的全纯CFT。

Conclusion: 全纯共形场论与拓扑场论对偶性之间存在深刻联系，通过研究Schellekens分类的全纯理论可以系统发现新的拓扑对偶性，这些对偶性涉及非阿贝尔任意子凝聚，并可以推广到更一般的理论系列。
...
### [4] [Coulomb Branches of Noncotangent Type: a Physics Perspective](https://arxiv.org/abs/2512.23908)
*Mykola Dedushenko,Daniel Resnick*

Main category: hep-th

TL;DR: 研究3D N=4规范理论中半超多重态的库仑分支，通过半球配分函数方法处理非余切理论，解决奇偶异常和边界条件问题，推导库仑分支算符代数及其应用。


<details>
  <summary>Details</summary>
Motivation: 研究具有半超多重态的一般伪实表示（非余切理论）的3D N=4规范理论的库仑分支，这类理论在传统方法中难以处理，需要解决奇偶异常和边界条件与规范对称性的兼容性问题。

Method: 扩展半球配分函数方法到非余切物质，使用规范场的Dirichlet边界条件，仔细处理半超多重态的边界数据，推导实现单极子插入的移位/差分算符，利用HS^3配分函数作为库仑分支算符代数的自然表示模。

Result: 推导了SU(2)理论（包括半整数自旋表示）的库仑分支算符代数的生成元和关系，分析了库仑分支为y^2=z(x^2-1)的理论，计算了具有自旋-3/2半超多重态的A_n四元图的库仑分支，并验证了一般单极子-反单极子两点函数的相容性。

Conclusion: 成功将半球配分函数方法扩展到处理非余切理论，解决了奇偶异常和边界条件问题，建立了库仑分支算符代数的系统框架，为研究更广泛的3D N=4规范理论提供了有效工具。
...
### [5] [5D AGT conjecture for circular quivers](https://arxiv.org/abs/2512.23878)
*A. Mironov,A. Morozov,Sh. Shakirov*

Main category: hep-th

TL;DR: 该论文将自由场形式主义扩展到椭圆曲面上的q-Virasoro共形块，验证了与5维圆形quiver规范理论的瞬子配分函数以及缺陷配分函数的等价性。


<details>
  <summary>Details</summary>
Motivation: 自由场形式主义为研究共形块提供了有效工具，特别是通过Dotsenko-Fateev型积分表示。作者希望将这种方法扩展到椭圆曲面上的q-Virasoro共形块，以建立与5维规范理论配分函数的联系。

Method: 扩展自由场形式主义到椭圆曲面上的q-Virasoro共形块，使用多重Dotsenko-Fateev型积分表示。对于退化情况，通过特殊参数选择得到相应的积分表示。验证这些积分表示与5维圆形quiver规范理论的瞬子配分函数以及缺陷配分函数（Shiraishi函数）的等价性。

Result: 成功建立了椭圆曲面上q-Virasoro共形块与5维规范理论配分函数的对应关系：1）一般情况与瞬子配分函数等价；2）退化情况与缺陷配分函数（Shiraishi函数）等价。两种情况下都得到了验证和一致性结果。

Conclusion: 该研究为从积分表示推导Shiraishi函数的复杂方程开辟了新途径，尽管技术细节复杂且留待未来完成。这项工作扩展了AGT关系在椭圆曲面上的应用，深化了对共形块与规范理论对应关系的理解。
...
TL;DR: 该论文研究了Kadar-Yu代数的非半单表示理论，通过计算标准模的Gram矩阵行列式，推广了Temperley-Lieb代数的根式范式，并建立了与Brauer代数的积分范式之间的插值关系。


<details>
  <summary>Details</summary>
Motivation: Kadar-Yu代数作为连接Brauer代数和Temperley-Lieb代数的代数序列，其半单表示理论已被理解，但非半单表示理论一直未解决。该研究旨在填补这一空白，为理解这些代数的完整表示理论提供关键工具。

Method: 使用重排塔（ToR）的同调工具，计算标准模的逆变形式的Gram矩阵的行列式，得到类似Chebyshev的广义形式。这些Gram行列式为研究标准模之间的态射提供了丰富信息。

Result: 确定了标准模Gram矩阵行列式的广义Chebyshev形式，推广了Temperley-Lieb代数的根式范式，并在各种方式上插值于Brauer代数的积分范式。这些结果为理解KY代数的非半单表示理论奠定了基础。

Conclusion: 该研究成功解决了Kadar-Yu代数非半单表示理论的关键问题，揭示了其表示理论具有强烈的alcove几何特征，并引导了对整体几何框架的有趣推广，为深入理解这类代数的表示理论提供了重要进展。
...
### [7] [Bilinear tau forms of quantum Painlevé equations and $\mathbb{C}^2/\mathbb{Z}_2$ blowup relations in SUSY gauge theories](https://arxiv.org/abs/2512.25051)
*Giulio Bonelli,Anton Shchechkin,Alessandro Tanzini*

Main category: math-ph

TL;DR: 该论文推导了正则量子化Painlevé方程的双线性tau形式，建立了与N=2超对称规范理论配分函数在Ω背景下的blowup关系之间的联系，并完善了Painlevé/规范理论对应字典。
...
Method: 推导正则量子化Painlevé方程的双线性tau形式，利用ℂ²/ℤ₂ blowup关系，构建量子非自治Painlevé哈密顿量的适当方程，并分析量子Painlevé tau函数的对称结构。

Result: 完全确定了精化Painlevé/规范理论对应字典，获得了量子非自治Painlevé哈密顿量的正确方程，并作为对称性分析的副产品，得到了规范理论在非平凡holonomy扇区中的ℂ²/ℤ₂ blowup关系。

Conclusion: 成功建立了量子Painlevé方程与N=2超对称规范理论之间的精确对应，通过双线性tau形式和blowup关系连接了两个理论，为理解这两个数学物理领域的深层联系提供了新视角。
...
### [8] [Charge functions for all dimensional partitions](https://arxiv.org/abs/2512.24343)
*Hao Feng,Tian-Shun Chen,Kilar Zhang*

Main category: math-ph

TL;DR: 该论文研究了n维分划的电荷函数，已知n=2,3,4的情况，作者在先前工作中给出了任意奇数维的表达式，现在进一步猜想所有偶数维情况的公式，并对6D进行了严格证明，对8D进行了数值验证。


<details>
  <summary>Details</summary>
Motivation: 研究n维分划的电荷函数，填补偶数维情况下公式的空白，扩展对高维分划电荷函数的理解。

Method: 基于已知的n=2,3,4维电荷函数，在先前给出任意奇数维表达式的基础上，提出所有偶数维情况的猜想公式，并对6维情况进行严格数学证明，对8维情况进行数值验证。

Result: 成功提出了偶数维分划电荷函数的猜想公式，对6维情况给出了严格证明，对8维情况通过数值验证支持了猜想的正确性。

Conclusion: 该研究为n维分划的电荷函数提供了完整的理论框架，奇数维已有表达式，偶数维提出了猜想公式并通过6维证明和8维验证支持其正确性，为高维分划理论的发展奠定了基础。
...
### [9] [Adjoint L-Infinity Actions and Conserved Charges in GR](https://arxiv.org/abs/2512.23970)
*Changsun Choi,Ryan E. Grady*

Main category: math-ph

TL;DR: 该论文提出了一种基于L∞-代数的新方法，用于计算爱因斯坦-嘉当-帕拉蒂尼引力理论中与无穷小等距（Killing场）相关的守恒流和荷，并应用于计算史瓦西黑洞熵。


<details>
  <summary>Details</summary>
Motivation: 研究爱因斯坦-嘉当-帕拉蒂尼引力理论中与等距对称性相关的守恒量和荷的计算问题，特别是为黑洞熵等物理量的计算提供新的数学框架。

Method: 采用L∞-代数形式体系和Ćirić、Giotopoulos、Radovanović、Szabo以及Costello和Gwilliam的工作，提出计算守恒流和荷的新方法，并证明了关于L∞-代数伴随作用的高阶（全∞）版本存在性和实用性的代数结果。

Result: 成功计算了史瓦西黑洞的熵，验证了所提方法的有效性，并建立了L∞-代数框架下守恒量计算的新理论工具。

Conclusion: L∞-代数为引力理论中对称性相关守恒量的计算提供了有力的数学框架，该方法不仅适用于爱因斯坦-嘉当-帕拉蒂尼引力，还可推广到其他物理系统，为黑洞热力学等研究提供了新的理论工具。
...
### [10] [Sub-structure in module category of Virasoro vertex operator algebras $L(c_{p,q}, 0)$](https://arxiv.org/abs/2512.23980)
*Qirui Fang,Yu Teng,Yukun Xiao,Wen Zheng*

Main category: math.QA

TL;DR: 本文确定了Virasoro顶点算子代数L(c_{p,q},0)的模范畴中所有预模子范畴和模张量子范畴，以及L(c_{p,p+1},0)的简单流扩张的模范畴中的相应子范畴。


<details>
  <summary>Details</summary>
Motivation: 研究Virasoro顶点算子代数模范畴的结构，特别是其中的预模子范畴和模张量子范畴的完全分类，这对于理解这些代数的表示理论和相关物理模型具有重要意义。

Method: 通过分析Virasoro顶点算子代数L(c_{p,q},0)的表示理论，研究其模范畴的范畴性质，确定所有满足预模条件的子范畴，并进一步识别其中的模张量子范畴。同时考虑L(c_{p,p+1},0)的简单流扩张的模范畴。

Result: 完全确定了Virasoro顶点算子代数L(c_{p,q},0)模范畴中所有预模子范畴和模张量子范畴，以及L(c_{p,p+1},0)简单流扩张模范畴中的相应子范畴，给出了完整的分类结果。

Conclusion: 本文完成了对Virasoro顶点算子代数及其简单流扩张的模范畴中预模子范畴和模张量子范畴的完全分类，为相关代数结构和物理模型的研究提供了重要基础。
...
### [11] [Generalized Symmetries From Fusion Actions](https://arxiv.org/abs/2508.13063)
...
<details>
  <summary>Details</summary>
Motivation: 研究模张量范畴中可凝聚代数上的融合作用，旨在建立范畴化的Schur-Weyl对偶理论，并探索融合子范畴与可凝聚子代数之间的对应关系，为顶点算子代数理论提供新的视角。

Method: 在模张量范畴中定义可凝聚代数A上的融合作用，利用A-模范畴C_A在态射空间Hom_C(x,A)上的作用，证明范畴化的Schur-Weyl对偶，建立融合子范畴与可凝聚子代数之间的伽罗瓦对应，并在顶点算子代数框架下验证结果。

Result: 证明了融合作用在A上的Schur-Weyl对偶范畴化版本；建立了包含所有局部A-模的融合子范畴与A的可凝聚子代数之间的伽罗瓦对应；在顶点算子代数中验证了U=A^{C_A}的关系，特别地当U=A^G时，融合作用等价于G-作用。

Conclusion: 该工作成功建立了模张量范畴中可凝聚代数上的融合作用理论，实现了Schur-Weyl对偶的范畴化，并揭示了融合子范畴与可凝聚子代数之间的深刻对应关系，为顶点算子代数的研究提供了新的数学工具。
...
### [12] [Isomorphism between Hopf algebras for multiple zeta values](https://arxiv.org/abs/2512.24732)
*Li Guo,Hongyu Xiang,Bin Zhang*

Main category: math.NT

TL;DR: 本文证明了多重zeta值的准洗牌代数和洗牌代数作为Hopf代数是同构的
...
Method: 通过代数结构分析，证明多重zeta值的准洗牌代数和洗牌代数作为Hopf代数是同构的。

Result: 成功证明了多重zeta值的准洗牌代数和洗牌代数作为Hopf代数是同构的。

Conclusion: 多重zeta值的两种Hopf代数结构（准洗牌代数和洗牌代数）实际上是同构的，这深化了我们对多重zeta值代数结构的理解。
...
### [13] [Quantum Expander Mixing Lemma and its Structural Converse](https://arxiv.org/abs/2403.11454)
*Ning Ning*

Main category: quant-ph

TL;DR: 该论文建立了经典扩展图混合引理的量子类比及其结构逆定理，为量子扩展器提供了理论工具


<details>
  <summary>Details</summary>
Motivation: 经典扩展图在计算机科学和数学中有广泛应用，随着量子技术的发展，量子扩展器在量子信息理论、量子复杂性和非交换伪随机性中找到了众多应用。经典扩展图混合引理在图论中起着核心作用，为分析网络性质和算法提供了重要工具，因此需要建立其量子类比。

Method: 建立经典扩展图混合引理的量子类比，并证明其结构逆定理，为量子扩展器提供类似的理论框架

Result: 成功建立了量子扩展器的混合引理及其结构逆定理，为量子扩展器的分析和应用提供了理论基础

Conclusion: 该研究为量子扩展器理论提供了重要工具，类似于经典扩展图混合引理在经典图论中的作用，将有助于量子信息理论、量子复杂性和相关领域的发展
